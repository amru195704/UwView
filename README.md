...
現在の安定版: **v1.2.1**（About 表示）。フェーズ0〜4（基盤・巨大ファイル閲覧）に加え、v1.1／v1.1.1 でログ解析向けの機能を追加し、v1.2.0 で UI の全面アイコン化と不具合修正（セッション復元ほか）、v1.2.1 でブラウザ版（WASM）の表示不具合修正と検索結果一覧の高速化を行いました。
