...
| やること | 相手 | 相手 | **UwView** | |
|---|---|---:|---:|:---:|
| **探す**（CLI） | ripgrep 15.2.0 | 55.38秒 | **`uvf` 50.82秒** | 同等 |
| **開く**（GUI） | klogg 24.11.0 | 52.55秒 | **50.44秒** | 同等 |
| **探して、画面で読むまで** | `rg` ＋ klogg | 107.9秒 | **`uvf … -open` 53.69秒** | **2.01倍** |
| **同じファイルに2問目** | klogg | 108.14秒 | **`uvp` 6.41秒** | **16.9倍** |

**探す速さは ripgrep と、開く速さは klogg と同等です。差がつくのは、その2つをつないだときです。**

`rg` で探してビューアで開き直すと、**ファイルを2回読みます。** `uvf … -open` は **1回**です。

> `rg`＋klogg の 107.9秒は、実測2本（55.38秒・52.55秒）の足し算です。klogg 単体（開く＋検索）は 108.14秒。
...
| | 3GB | 10GB | 50GB |
|---|---:|---:|---:|
| **探す** ripgrep（検索7種の合計） | **32.30秒** | 158.69秒 | 806.22秒 |
| **探す** `uvf` | 33.31秒 | **147.01秒** | **735.97秒** |
| **開く** klogg | 3.65秒 | 10.98秒 | 52.55秒 |
| **開く** UwView | **2.99秒** | **10.13秒** | **50.44秒** |

**3GB では ripgrep に負けています**（メモリに載るので2回目がキャッシュから返るため）。
10GB を超えると `uvf` が 8〜10% 前に出ます。**どちらも律速はディスクの読み出し速度**です。
