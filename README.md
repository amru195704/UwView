...
- 🚀 **超高速な巨大ファイル表示** — 45億行クラスを省メモリで閲覧（実測最大 258.68GB / 4,509,830,821行）。ファイル本体は非常駐、索引は約6MB（2億行時）。
