...
| 検索 literal `"Tokyo"` | 約585秒 ／ 120〜135秒 ／ 15〜20秒 | **74.8秒 ／ 14.3秒 ／ 5.1秒** | **約7.8倍 ／ 約9倍 ／ 3〜4倍** |
| 検索 regex `"Tok[yi]o"` | ≒literal（I/O律速でパターン非依存） | USB SSD **29.8秒** ／ 内蔵SSD **11.0秒** | 約4.4倍 ／ 約1.5倍 |
| ディスク占有（アーカイブ運用） | 48GB（元ファイル必須） | **5.3GB**（元ファイル削除可・チェックサム保護付き） | **1/9** |

- **klogg は索引作成中、先頭しか見えません**（中間・末尾へ移動不可）。UwView はバイト位置ベース表示のため、開いた直後から全域へ自由に移動できます。
- **ディスクが遅いほど差が開きます**（読むバイト数を1/9にするため）。HDD では検索10分→75秒。
...
- 🧵 **検索結果一覧ポップアップ** — ヒット行だけを別ウィンドウに一覧表示（元ファイル不変・仮想ビュー・元の行番号付き）。ダブルクリック/Enter で本文の該当行へジャンプ、結果は**別ファイルへ保存**可能（行番号の有無を選択）。Pro 版はさらに**前後±N行の文脈表示**とタブごとのウィンドウに対応。
