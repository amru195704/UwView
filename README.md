...
実際はは**DISKファイルサイズ**の上限まででしょう。きっと。


![UwView — OSM日本 51GB・8.9億行を行モードで表示](press-kit/screenshots/line-mode.png)

かつて Vector で公開していた大容量テキストビューア UwView を、[Avalonia UI](https://avaloniaui.net/) で再作成するプロジェクトです。通常のエディタは100万行程度で開けなくなりますが、UwView はファイル全体をメモリに載せず、**見えている行だけを描画**することで巨大ファイルを実用的に閲覧できます。RDB/XML ダンプ等で発生する巨大行数ファイルを「とにかく速く見る」ことに特化しています。現在の最大行数テストは8.9億行（＝OpenStreetMapの日本全体地図ソースで51Gbyte程度のxmlファイルです）。誰か限界を調査したら教えてください（できるものなら。私は怖くてできません）。

エディタではなく **ビューア**（閲覧専用）です。

## 📣 お知らせ：さらに高速な「UwView Pro」開発中

大容量ファイルをさらに高速化した **UwView Pro** を開発しています。索引の並列構築に加え、**圧縮サイドカーキャッシュ**（`.uwvz`＋`.uwvidx`）により「2回目以降の瞬間オープン」と「全文検索の大幅高速化」を実現します。

著名な大容量ログビューア **[klogg](https://klogg.filimonov.dev/)** との実測比較（同一ファイル・同一パターン）:

> 条件: OpenStreetMap 日本全域 `japan-latest.osm` 47.73GB・892,239,125 行、外付けUSB（実測物理帯域 0.41GB/s）、RAM 32GB・10コア Mac、Match case ON。検索のヒット数は全項目で klogg・UwView Pro・生ファイル直接検索の間で**厳密一致**を確認済み（＝意味的に同じ検索であることを相互検証）。

| 項目 | klogg (24.11.0) | UwView 公開版 | **UwView Pro（開発中）** | Pro ÷ klogg |
|---|---|---|---|---|
| 初回オープン | 索引構築 約110秒（完了まで**先頭しか見えない**・中間/末尾へ移動不可） | 索引構築 128〜186秒（**全域即表示**） | 140秒で索引＋圧縮キャッシュ生成（**全域即表示**） | — |
| 2回目以降のオープン | 毎回再索引 約110秒（永続キャッシュ無し） | 毎回再構築 128〜186秒 | **0.02〜0.07秒** | **1,500倍以上** |
...
| ディスク占有（アーカイブ運用） | 48GB（元ファイル必須） | 48GB（元ファイル必須） | **5.3GB**（元ファイル削除可・チェックサム保護付き） | **1/9** |

- **klogg は索引作成中、先頭しか見えません**（中間・末尾へ移動不可）。UwView はバイト位置ベース表示のため、開いた直後から全域へ自由に移動できます。
- 圧縮サイドカーにより、元ファイルを削除して **約 1/9 サイズ**（例: 48GB→5.3GB）で保管し、そのまま閲覧することも可能です（チェックサム保護付き）。

> UwView Pro は商用（有償）版として準備中です。続報をお待ちください。

## 特長

- 🚀 **超高速な巨大ファイル表示** — 8.9億行クラスを省メモリで閲覧。ファイル本体は非常駐、索引は約6MB（2億行時）。
...
| 項目 | 実測値 |
|------|--------|
| サイズ | 51,254,526,392 バイト（約48 GiB） |
| open＋文字コード判定 | 12 ms |
| ページモード表示 | 3 ms（索引を待たない） |
| 索引構築 | 172.8 s（283 MB/s・外付けSSD律速） |
| 総行数 | **892,239,125**（`wc -l` と完全一致） |
| 索引サイズ | チェックポイント 3,485,310件 ≒ 26.6 MB |
| マネージヒープ増 | 33.3 MB |
| GetLine ランダム1000回 | 平均 1.28 ms / p99 3.0 ms |
| 末尾（8.9億行目）ジャンプ | 0.006 ms |

- **8.9億行超でも破綻せず**、総行数は `wc -l` と1行の狂いもなく一致。末尾（8.9億行目）ジャンプも 0.006 ms。行番号は `long` 通し（int 上限≒21億にも収まる）。
- GetLine が合成データ（0.005 ms）より遅い（1.28 ms）のは、外付けSSDのランダム読み・実XMLの行長不均一・51GBがキャッシュに載りきらない実I/Oが理由。ms級で実用範囲。
- 常駐は索引26.6MB＋ヒープ33MBのみ。行数よりストレージ容量が先に効く、という実データ特性が確認できた。
...
| **GeoDiveExa** | RTK-GNSS 対応の高精度位置調査アプリ（原点となるメインアプリ） | [公式サイト](https://y42u.net/tec001/) |

> UwView は上記 Geo 製品群とは独立した純粋なユーティリティで、Geo 資産への依存はありません。
...
**自己責任**でご利用ください。本ソフトを使用した結果いかなるトラブルが発生しても、作者は一切の責任を負いません。
